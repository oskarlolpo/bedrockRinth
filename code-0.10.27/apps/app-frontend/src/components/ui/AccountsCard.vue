<template>
	...
	<transition name="fade">
		...
		>
			...
			<Button v-if="accounts.length > 0" color="raised" @click="showOfflineLoginModal()">
				<PlusIcon />
				Add offline account
			</Button>
		</Card>
...
async function addOfflineProfile() {
	...
	if (!isValidName) {
		handleError(
			new Error(
				`Invalid offline name. Allowed: a-z A-Z 0-9 _, length ${minOfflinePlayerNameLength}-${maxOfflinePlayerNameLength}`,
			),
		)
		return
	}
