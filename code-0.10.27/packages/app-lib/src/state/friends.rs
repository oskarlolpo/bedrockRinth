        ...
        if let Some(credentials) = credentials {
            ...
            match res {
                ...
                Err(e) => {
                    ...
                    let is_dns_not_found = err_text.contains("11001")
                        || err_text
                            .to_ascii_lowercase()
                            .contains("host unknown")
                        || err_text.contains("Этот хост неизвестен");

                    if is_dns_not_found {
                        tracing::warn!(
                            "Friends socket host is not resolvable right now: {e:?}"
                        );
                    } else {
                        tracing::error!(
            ...
            loop {
                        ...
                        > chrono::Duration::seconds(30)
                {
                        ...
                        .await
                    {
                        ...
                        if err_text.contains("11001")
                            || err_text
                                .to_ascii_lowercase()
                                .contains("host unknown")
                            || err_text.contains("Этот хост неизвестен")
                        {
                            // DNS name is not resolvable; back off to reduce log spam.
                            last_connection =
