...
import { get_bedrock_versions, get_loader_versions } from '@/helpers/metadata'
...
const [
	...
	get_bedrock_versions()
		.then((versions: BedrockVersionTuple[]) => shallowRef(versions))
		.catch((err) => {
			handleError(err)
			return shallowRef([])
		}),
...
if (loaders?.value && !loaders.value.some((x) => x.name === 'bedrock')) {
	loaders.value.push({ name: 'bedrock', icon: null } as PlatformTag)
}
...
const hasSnapshots = computed(() => {
	...
	return gameVersionsForLoader.value?.some((x) => x.version_type !== 'release')
})

const selectableGameVersionNumbers = computed(() => {
	if (loader.value === 'bedrock') {
		return [...new Set((bedrock_versions?.value ?? []).map((x) => x[0]))]
	}
...
const selectedBedrockVersion = computed(() => {
	if (loader.value !== 'bedrock') {
		return null
	}

	return (bedrock_versions?.value ?? []).find((item) => item[0] === gameVersion.value) ?? null
})
...
async function saveGvLoaderEdits() {
	...
	if (loader.value === 'bedrock') {
		if (selectedBedrockVersion.value) {
			editProfile.loader_version = `${selectedBedrockVersion.value[1]}_${selectedBedrockVersion.value[2]}`
		}
	} else if (loader.value !== 'vanilla' && loaderVersionIndex.value !== undefined) {
...
<template>
	...
	<div>
				...
				<div class="flex gap-2 items-center">
					...
					<div class="flex flex-col gap-2 justify-center">
						...
						<span class="text-sm text-secondary leading-none">
							...
							<template
								v-if="
									instance.loader !== 'vanilla' && instance.loader !== 'bedrock' && !modpackProject
								"
							>
		...
		<template v-if="!instance.linked_data || !instance.linked_data.locked">
			...
			<template v-if="loader !== 'vanilla' && loader !== 'bedrock'">
				<h2 class="m-0 mt-4 text-lg font-extrabold text-contrast block">
