...
use crate::state::{
    CacheBehaviour, CachedEntry, Credentials, JavaVersion, ModLoader,
...
pub async fn kill(path: &str) -> crate::Result<()> {
    let state = State::get().await?;
    if let Some(profile) = get(path).await?
        && profile.loader == ModLoader::Bedrock
    {
        // Bedrock is launched via shell UWP activation and is not managed as a java child process.
        // Stop button should still terminate the running Bedrock process.
        #[cfg(windows)]
        {
            let script = "$ErrorActionPreference='SilentlyContinue'; Get-Process -Name 'Minecraft.Windows' | Stop-Process -Force";
        ...
        let _ = crate::launcher::bedrock::sync_profile_data_from_system(&profile).await;
    }
