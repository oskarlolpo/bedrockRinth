...
impl Profile {
    ...
    pub async fn remove_project(
        ...
        if let Ok(path) = crate::api::profile::get_full_path(profile_path).await
        {
            ...
            let mut candidates = vec![path.join(project_path)];
            ...
            // Optional fallback for disabled-path toggles.
            if !normalized.ends_with(".disabled") {
                let mut disabled_candidates = Vec::with_capacity(candidates.len());
                for candidate in &candidates {
                    ...
                    if !disabled_name.is_empty() {
                        disabled_candidates.push(candidate.with_file_name(disabled_name));
                    }
                }
                candidates.extend(disabled_candidates);
            }
                ...
                if let Ok(meta) = tokio::fs::symlink_metadata(&candidate).await {
                    if meta.file_type().is_dir() {
                        tokio::fs::remove_dir_all(&candidate)
                            .await
                            .map_err(|e| io::IOError::with_path(e, &candidate))?;
                    } else {
