...
async fn get_all_worlds_in_profile(
    ...
    let mut worlds = vec![];
    let state = State::get().await?;
    let profile = Profile::get(profile_path, &state.pool).await?;
    let is_bedrock = profile
        .as_ref()
        .map(|p| p.loader == ModLoader::Bedrock)
        .unwrap_or(false);

    get_singleplayer_worlds_in_profile(profile_dir, is_bedrock, &mut worlds).await?;
...
async fn get_singleplayer_worlds_in_profile(
    ...
    let mut saves_dir = if is_bedrock {
        instance_dir
            .join("bedrock")
            .join("com.mojang")
    ...
    if is_bedrock && (!saves_dir.exists() || !saves_dir.is_dir()) {
        if let Some(system_worlds) = current_bedrock_system_worlds_dir() {
            saves_dir = system_worlds;
        }
    }
    ...
    while let Some(world_dir) = saves_dir.next_entry().await? {
        ...
        let read_result = if is_bedrock {
            read_bedrock_singleplayer_world(world_path).await
        } else {
            let level_dat_path = world_path.join("level.dat");
            if !level_dat_path.exists() {
                continue;
            }
            read_singleplayer_world(world_path).await
        };
        if let Ok(world) = read_result {
            worlds.push(world);
        }
    }
...
fn current_bedrock_system_worlds_dir() -> Option<PathBuf> {
...
async fn read_bedrock_singleplayer_world(world_path: PathBuf) -> Result<World> {
    let levelname_path = world_path.join("levelname.txt");
    let world_name = if levelname_path.exists() {
        tokio::fs::read_to_string(&levelname_path)
            .await
    ...
    let icon = [
        world_path.join("world_icon.jpeg"),
        world_path.join("world_icon.jpg"),
        world_path.join("world_icon.png"),
    ]
    .into_iter()
    .find(|p| p.exists());

    let last_played = if let Ok(meta) = io::metadata(&levelname_path).await {
        meta.modified().ok().map(DateTime::<Utc>::from)
    } else if let Ok(meta) = io::metadata(&world_path).await {
        meta.modified().ok().map(DateTime::<Utc>::from)
    } else {
        None
    };
...
pub async fn get_profile_protocol_version(
    ...
    let game_version_lower = profile.game_version.trim().to_ascii_lowercase();
    let loader_version_lower = profile
        .loader_version
        .as_deref()
        .unwrap_or_default()
        .trim()
        .to_ascii_lowercase();
    let looks_like_bedrock_pkg = game_version_lower.ends_with(".appx")
        || game_version_lower.ends_with(".msix")
        || game_version_lower.ends_with(".appxbundle")
        || game_version_lower.ends_with(".msixbundle")
        || game_version_lower.ends_with(".msixvc")
        || loader_version_lower.ends_with(".appx")
        || loader_version_lower.ends_with(".msix")
        || loader_version_lower.ends_with(".appxbundle")
        || loader_version_lower.ends_with(".msixbundle")
        || loader_version_lower.ends_with(".msixvc")
        || loader_version_lower.contains(".appx_")
        || loader_version_lower.contains(".msixvc_")
        || loader_version_lower
            .contains("github.com/onixclient/onix_compatible_appx/releases/download/");
    if profile.loader == ModLoader::Bedrock || looks_like_bedrock_pkg {
        return Ok(None);
    }
