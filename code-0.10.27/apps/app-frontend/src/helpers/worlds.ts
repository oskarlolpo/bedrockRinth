...
export async function showWorldInFolder(instancePath: string, worldPath: string) {
	...
	const javaPath = fullPath + '/saves/' + worldPath
	const bedrockPath = fullPath + '/bedrock/com.mojang/minecraftWorlds/' + worldPath
	try {
		return await openPath(javaPath)
	} catch {
		return await openPath(bedrockPath)
	}
}
