    ...
    if let Ok(profiles_dir) = std::fs::read_dir(dirs.profiles_dir()) {
        for profile_dir in profiles_dir {
                ...
                && let Some(file_name) = file_name.to_str()
            {
                ...
                watch_profile(file_name, None, watcher, dirs).await;
            }
        }
    }
}
...
pub(crate) async fn watch_profile(
    profile_path: &str,
    is_bedrock_hint: Option<bool>,
    watcher: &FileWatcher,
    dirs: &DirectoryInfo,
) {
    let profile_path = dirs.profiles_dir().join(profile_path);
    if profile_path.exists() && profile_path.is_dir() {
        // Do not call profile API here: this function is also used during state initialization.
        // We detect Bedrock profile by on-disk layout.
        let bedrock_root = profile_path.join("bedrock").join("com.mojang");
        let is_bedrock = is_bedrock_hint
            .unwrap_or_else(|| bedrock_root.exists() || profile_path.join("bedrock").exists());

        if is_bedrock {
            if !bedrock_root.exists()
                && let Err(e) =
                    crate::util::io::create_dir_all(&bedrock_root).await
            {
                tracing::error!(
                    "Failed to create bedrock watcher directory {bedrock_root:?}: {e}"
                );
                return;
            }
            ...
            if let Err(e) = watcher
                .watcher()
                .watch(&bedrock_root, RecursiveMode::Recursive)
            {
                tracing::error!(
                    "Failed to watch bedrock directory {bedrock_root:?}: {e}"
                );
                return;
            }
        } else {
