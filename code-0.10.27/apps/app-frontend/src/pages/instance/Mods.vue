			...
			<div class="flex items-center gap-2 mb-4">
				<div class="iconified-input flex-grow">
					...
					<input
						...
						type="text"
						:placeholder="formatMessage(messages.searchProjects, { count: filteredProjects.length })"
			...
			>
				...
				<template #actions="{ item }">
					...
					<ButtonStyled type="transparent" circular>
						<button v-tooltip="formatMessage(messages.remove)" @click="removeMod(item)">
							<TrashIcon />
						</button>
					</ButtonStyled>
...
import { get_installed_bedrock_content } from '@/helpers/metadata.js'
...
const initProjects = async (cacheBehaviour?: CacheBehaviour) => {
	...
	if (props.instance.loader === 'bedrock') {
		const installed = (await get_installed_bedrock_content(props.instance.path).catch(handleError)) as
			| BedrockInstalledEntry[]
			| undefined
		for (const entry of installed ?? []) {
			if (entry.kind === 'world') {
				// Bedrock worlds should be displayed in the dedicated "Worlds" tab.
				continue
			}
			const exists = newProjects.some((x) => x.path === entry.rel_path || x.file_name === entry.file_name)
			if (exists) continue
			newProjects.push({
				path: entry.rel_path,
				name: entry.name,
				...
				icon: entry.image_url ?? undefined,
				disabled: false,
				outdated: false,
				updated: dayjs(0),
				project_type: entry.kind === 'world' ? 'world' : 'resourcepack',
			})
		}
	}
...
const messages = defineMessages({
	...
	searchProjects: {
		id: 'instance.content.search-projects',
	...
	shareProjectNames: {
		id: 'instance.content.share.project-names',
		defaultMessage: 'Project names',
	},
	shareFileNames: {
		id: 'instance.content.share.file-names',
		defaultMessage: 'File names',
	},
	shareProjectLinks: {
		id: 'instance.content.share.project-links',
		defaultMessage: 'Project links',
	},
	shareMarkdownLinks: {
		id: 'instance.content.share.markdown-links',
	...
	updateAll: {
		id: 'instance.content.update-all',
		defaultMessage: 'Update all',
	},
	updatePack: {
		id: 'instance.content.update-pack',
