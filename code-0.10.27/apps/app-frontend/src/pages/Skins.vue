...
const messages = defineMessages({
	...
	changeCape: {
		id: 'skins.change-cape',
		defaultMessage: 'Change cape',
	},
	skinOverridesCape: {
		id: 'skins.skin-overrides-cape',
	...
	pleaseSignIn: {
		id: 'skins.signin.title',
		defaultMessage: 'Please sign-in',
	},
	pleaseSignInBody: {
		id: 'skins.signin.body',
		defaultMessage:
	...
	slowDownTitle: {
		id: 'skins.slow-down.title',
		defaultMessage: 'Slow down!',
	},
	skinRateLimit: {
		id: 'skins.slow-down.skin',
		defaultMessage:
	...
	try {
		...
		if ((error as { message?: string })?.message?.includes('429 Too Many Requests')) {
			notifications.addNotification({
				type: 'error',
				title: formatMessage(messages.slowDownTitle),
				text: formatMessage(messages.skinRateLimit),
			})
		} else {
			handleError(error as Error)
		}
	...
	try {
		...
		if ((error as { message?: string })?.message?.includes('429 Too Many Requests')) {
			notifications.addNotification({
				type: 'error',
				title: formatMessage(messages.slowDownTitle),
				text: formatMessage(messages.capeRateLimit),
			})
		} else {
			handleError(error as Error)
		}
...
<template>
	...
	<ConfirmModal
		...
		:proceed-label="formatMessage(messages.deleteSkinProceed)"
		@proceed="deleteSkin"
	/>
							...
							<button
								v-tooltip="
									selectedSkin?.cape_id
										? formatMessage(messages.skinOverridesCape)
										: undefined
								"
				...
				<div class="skin-card-grid">
					...
					>
						<template #overlay-buttons>
							<Button
								color="brand"
								:aria-label="formatMessage(messages.editSkin)"
							...
							<Button
								...
								v-tooltip="formatMessage(messages.deleteSkin)"
								:aria-label="formatMessage(messages.deleteSkin)"
								color="red"
	...
	<div v-else class="flex items-center justify-center min-h-[50vh] pt-[25%]">
		...
		>
			...
			<div class="flex flex-col gap-5">
				...
				<p class="text-lg m-0">
					{{ formatMessage(messages.pleaseSignInBody) }}
				</p>
