...
export async function create_bedrock_profile_from_package_file(path) {
	const imported = await invoke('plugin:utils|import_bedrock_package_file', { path })
	const profile = await create(
		imported.suggestedName,
		imported.gameVersion,
		'bedrock',
		imported.loaderVersion,
		null,
		false,
	)
	return profile
