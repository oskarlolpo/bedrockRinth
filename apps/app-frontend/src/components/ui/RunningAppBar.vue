	...
	<div class="action-groups">
		...
		<div v-if="offline" class="status">
			...
			<div class="running-text">
				<span> {{ formatMessage(messages.offline) }} </span>
			</div>
		</div>
		...
		<div v-else class="status">
			...
			<span class="running-text"> {{ formatMessage(messages.noInstancesRunning) }} </span>
		</div>
	</div>
...
const messages = defineMessages({
	offline: {
		id: 'app.running-bar.offline',
		defaultMessage: 'Offline',
	},
	stopInstance: {
		id: 'app.running-bar.stop-instance',
		defaultMessage: 'Stop instance',
	},
	viewLogs: {
		id: 'app.running-bar.view-logs',
		defaultMessage: 'View logs',
	},
	noInstancesRunning: {
		id: 'app.running-bar.no-instances',
		defaultMessage: 'No instances running',
	},
	downloadingJava: {
		id: 'app.running-bar.downloading-java',
	...
	currentLoadingBars.value = Object.values(await progress_bars_list().catch(handleError))
		.map((x) => {
			if (x.bar_type.type === 'java_download') {
				x.title = formatMessage(messages.downloadingJava, { version: x.bar_type.version })
			}
