...
import FirstRunBedrockSetupModal from '@/components/ui/modal/FirstRunBedrockSetupModal.vue'
...
import { provideAppUpdateDownloadProgress } from './providers/download-progress'
...
// Keep a no-op provider so settings modal dependencies stay valid
// while launcher self-update is disabled.
...
const firstRunBedrockSetupModal = ref()
...
const messages = defineMessages({
	...
	navDiscover: {
		id: 'app.nav.discover-content',
		defaultMessage: 'Discover content',
	},
	navSkinsBeta: {
		id: 'app.nav.skins-beta',
...
async function setupApp() {
	...
	themeStore.initializeAccentState()
	...
	if (showOnboarding.value) {
		await nextTick()
		firstRunBedrockSetupModal.value?.show()
	}
...
// Launcher self-update flow is intentionally disabled in this build.
...
<template>
	...
	>
		...
		<Suspense>
			<FirstRunBedrockSetupModal ref="firstRunBedrockSetupModal" />
		</Suspense>
		<Suspense>
		...
		>
			...
			<NavButton v-tooltip.right="formatMessage(messages.navSkinsBeta)" to="/skins">
				<ChangeSkinIcon />
			</NavButton>
			<NavButton
			...
			<OverflowMenu
				v-if="credentials"
				v-tooltip.right="formatMessage(messages.accountMenuTitle)"
			...
			<NavButton
				v-else
				v-tooltip.right="formatMessage(messages.navSignIn)"
				:to="() => signIn()"
			>
			...
			<transition name="popup-survey">
				...
				>
					...
					<p class="m-0 leading-tight">
						{{ formatMessage(messages.surveyBody1) }}
					</p>
					<p class="mt-3 mb-4 leading-tight">
						{{ formatMessage(messages.surveyBody2) }}
					</p>
